...
void acbutton_chtxt(ACONTROLP ctl, char * text, byte isbig) {
  ...
  if (!atheme_draw("img.button.focus", &d->control_focused,0,0,w,h)){
    ...
    ag_roundgrad(&d->control_focused,1,1,w-2,h-2,acfg()->selectbg,acfg()->selectbg_g,(agdp()*acfg()->btnroundsz)-1);
    ag_roundgrad_ex(&d->control_focused,1,1,w-2,(h-2)/2,LOWORD(hl1),HIWORD(hl1),(agdp()*acfg()->btnroundsz)-1,1,1,0,0);
  }
