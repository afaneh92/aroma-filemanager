 ...
 */
byte
auido_next_cut_size(AFSDTP dt, int * curr_id, byte * copy_status,
                    char ** source_path, char ** dest_path, int number_files) {
  int i;
  ...
  for (i = *curr_id; i < number_files; i++) {
    if (copy_status[i] == 0) {
      ...
      if (file_exists(dest_path[i])) {
        do_full_move = 1;
      }
      else if (rename(source_path[i], dest_path[i]) == -1) {
        do_full_move = 1;
      }
      ...
      if (do_full_move) {
        *curr_id = i;
        ...
        if (atouch_send_message(aw_msg(11, 0, 0, 0))) {
          *curr_id = i;
          char * bsn = basename(source_path[i]);
          
          if (bsn != NULL) {
            snprintf(dt->curr, 255, "%s",
                     bsn);
            free(bsn);
          }
...
void
auido_show_cut(byte * copy_status, char ** source_path, char ** dest_path,
               int number_files) {
  //-- Init Dialog Window
  ...
  AWINDOWP hWin = aw(&bg);
  ACONTROLP mainInfo =
    aclabel(hWin, cliX, titY, cliW, titH, alang_get("move.prepare"), 1,
            1,
            2, acfg()->winfg);
  ACONTROLP extraInfo =
  ...
  if (auido_next_cut_size
      (&dt, &curr_id, copy_status, source_path, dest_path, number_files)) {
    //-- Dispatch
    do {
        ...
        case 11: {
            ...
            if (proc_state == 0) {
              ...
              if (!auido_next_cut_size
                ...
                if (!auido_next_copy_action(&ct, &copy_id, copy_status, source_path, dest_path, number_files, 1)) {
                  ct.flag = 0;
                  ondispatch = 0;
                }
                else {
...
void auido_cut(char * dest, byte * tool_state) {
  ag_setbusy();
  ...
  if (auic_s > 0) {
    ...
    if (not_valid) {
      ...
      auido_show_cut(copy_status, source_path, dest_path,
                     number_files);
      release_clipboard = 1;
    }
    else {
