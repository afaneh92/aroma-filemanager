...
byte ag_draw_strecth(CANVAS * d,
  ...
  for (i = 0; i < dh; i++) {
    word * t = d->data + (i + dy) * d->w + dx;
    y2       = ((i * y_ratio) >> 16);
    word * p = s->data + (y2 + sy) * s->w + sx;
    int rat = 0;
        ...
        for (j = 0; j < crw; j++) {
          ...
          if (xpos >= 0) {
            if (xpos < ((int)ag_fbsz - 2)) {
              ...
              byte ng     = 255 - ag_g(fbc);
              byte nb     = 255 - ag_b(fbc);
              ag_fbuf[xp] = ag_rgb(nr, ng, nb);
            }
          }
        }
      }
  ...
  *y        = 0;
  ...
  while (*s != 0) {
    ...
    if (charp <= 0) {
      ...
      bf[line_width + charp] = 0;
      *x        = ag_txtwidth(bf, isbig);
      free(bf);
      eos = 1;
    }
    else if (eos) {
